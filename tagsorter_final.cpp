...
//Functions
void cleanup(string &source);//Used to scrub commas and other things from inputs
...
int main()
{
    ...
    string capitals = "PCTAGLCP";//the first letter of each term in order of appearance
    ...
    do{
            ...
            while(!inFS.eof())//store all information in tempdisplay
            {
                ...
                if(posinwork == 0)
                {
                    ...
                    {        
                        ...
                        if(copy[1] == 'o')//Only fires if the code comes from missav
                        {       
                            posinwork++;
                            tempstring = tempdisplay[0];
                            while(tempstring[posinword] != '\0')//Skips the code and adds everything after it
                            {
                                posinword++;
                                if(tempstring[posinword] == ' ')
                                {
                                    posinword++;
                            ...
                            for(i = 0; i < 7; i++)//skips the irrelevant parts of the input
                            {
                                getline(inFS, copy);
                            }
                            posinword = 0;
                            ...
                            paste.clear();
                            tempstring.clear();
                            check2 = 1;//tells the program that a code is being scanned
                            break;
                        }
                        tempdisplay[k] = copy;
                        getline(inFS, copy);
                ...
                if(check2 == 0)//scan procedure from works
                {
                    cout << "detected code" << endl;
                ...
                else//scan procedure for other works
                {
                    if(posinwork == 2)//grab all information and save to tempdisplay
                    {   
                        if(copy[0] == 'L')
                        {
                            posinwork++;
                    ...
                    if(posinwork == 3)//write to sauce.txt
                    {
                        cout << tempdisplay[0] << " " << endl;//title
                        cout << tempdisplay[3] << " " << endl;//secondary title
                        cout << tempdisplay[2] << " " << endl;//code
                        cout << terms[0] << " " << "n/a " << endl;//parodies
                        cout << terms[1] << " " << tempdisplay[4] << " " << endl;//characters
                        cout << terms[2] << " " << tempdisplay[5] << jaxa " << endl;//tags
                        cout << terms[3] << " " << tempdisplay[7] << " " << endl;//artists
                        cout << terms[4] << " " << tempdisplay[6] << " "<< endl;//groups
                        cout << terms[5] << " Japanese " << endl;//langauges
                        cout << terms[6] << " jaxa " << endl;//categories
                        cout << terms[7] << " n/a " << endl;//pages
                        posinwork++;
                    }
                } 
            }
                ...
                if(check1 > 0)
                {
                    while(!inFS.eof())
                    {
                        ...
                        if(tempdisplay[2] == tempcodes[i])
                        {
                            ...
                            //open the folder
                            //system("D:/sebas/Documents/tagsorter/images/2");
        ...
        else if(functionselect == 4)//show collections
        {
            ...
            {
                cout << 
R"(Input the number corresponding to the desired term...
1 Parodies 
2 Characters
3 Tags 
4 Artists 
...
8 Pages)" << '\n';
                ...
                cin >> i;
                tempstring = terms[i-1];
                posinwork = i + 2;//tells the program which lines to scan and which to skip
                ...
                tempnumber = occurances[0];

                for(j = 0; j < mokuteki; j++)//find the character count of the largest item. We use this to format the output nicely. Also find the tag with the nost occurances
                {
                    l = tempstring.length();
                    m = results[j].length();
                    ...
                    if(m > l)
                    {
                        tempstring = results[j];
                    }
                    if(occurances[j] > tempnumber)
                    {
                        tempnumber = occurances[j];
                    }
                ...
                cout << setw(4+m) << left << "Popular";
                cout << " " << setw(4+m) << left << "Alphabetical" << endl;

                for(k = i; k > 0; k = k-1)//print everything to the terminal
                {
                    for(j = 0; j < mokuteki; j++)
                    {
                        if(occurances[j] == k)
                        {
                            cout << setw(4) << left << k; 
                            cout << setw(m) << left << tempresultsp[j] << " ";
                            cout << setw(4) << left << tempoccurances[l]; 
                            cout << setw(m) << left << results[l] << endl;
                            l++;
                        }
                    }
                }
        ...
        {
            cout << 
R"(This script is intended to be used with specified sites only. Support for other sites might come later
